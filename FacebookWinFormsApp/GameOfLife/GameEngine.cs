...
namespace BasicFacebookFeatures
{
    ...
    {
        private GameGrid k_GameBoard;

        public GameGrid CurrentlyBeingUsed
        {
            get => k_GameBoard;
            set => k_GameBoard = value;
        }
        ...
        public GameEngine(int i_GameRows, int i_GameColumns)
        {
            ...
            k_GameBoard = new GameGrid(i_GameRows, i_GameColumns);
        ...
        public void UpdateToNextGeneration()
        {
            ...
            for (int i = 0; i < k_GameRows; ++i)
            {
                ...
                {
                    tmp[i, j] = k_GameBoard.GameMatrix[i, j];
                }
            }
            ...
            for (int rowIndex = 0; rowIndex < k_GameRows; rowIndex++)
            {
                for (int columnIndex = 0; columnIndex < k_GameColumns; columnIndex++)
                {
                    ...
                    if (k_GameBoard.GameMatrix[rowIndex, columnIndex] == 1)
                    {
                        if (numOfAliveNeighbors < 2)
                        {
                            k_GameBoard.GameMatrix[rowIndex, columnIndex] = 0;
                        }
                    ...
                    else
                    {
                        if (numOfAliveNeighbors == 3)
                        {
                            k_GameBoard.GameMatrix[rowIndex, columnIndex] = 1;
                        }
                    }
                }
            }
